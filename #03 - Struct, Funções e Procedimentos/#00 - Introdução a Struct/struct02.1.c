 ...
 **/
 ...
 typedef struct {
	char nome[100], disciplina[100];
	int matricula, notaFinal;
 } Estudante;
int main() {
	...
	while (1) {
		...
		scanf(" %s", e[cont].disciplina);
		printf("Insira a nota final da disciplina:\n  - > ");
		scanf(" %d", &e[cont].notaFinal);
		cont++;
		if(cont > MAX_DB) {
	...
	while (1) {
		...
		printf("  MATRÍCULA Nº%d\n Nome: %s\n Disciplina: %s\n Nota Final: %d",
			e[busca].matricula, e[busca].nome, e[busca].disciplina, e[busca].notaFinal);
